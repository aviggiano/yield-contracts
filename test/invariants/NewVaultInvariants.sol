...
contract NewAsset is STETH {
    function decimals() public view virtual override returns (uint8) {
        return 6;
    }
}

contract NewVaultInvariants is PropertiesConstants, PropertiesAsserts {
    ConfigurationManager private $configuration = new ConfigurationManager();
    NewAsset private $asset = new NewAsset();
    InvestorActorMock private $investor = new InvestorActorMock(address($asset));
    STETHVault private vault = new STETHVault($configuration, $asset, address($investor), 10_000);
